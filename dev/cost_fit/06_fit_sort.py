...
def main() -> None:
    ...
    ap.add_argument("--data", default=os.path.join(os.path.dirname(__file__), "data", "sort_samples.jsonl"))
    ap.add_argument("--out", default=os.path.join(os.path.dirname(__file__), "models", "sort_coef.json"))
    ...
    names = ["actual_rows", "plan_width", "sort_external"]

    def xfn(r):
        return [r["actual_rows"], r["plan_width"], r["sort_external"]]

    ev = fit_train_test_eval(rows, names, xfn, train_n=args.train_n, test_n=args.test_n, seed=args.seed)
    payload = {"operator": "Sort", "features": names, "total_samples": len(rows), **ev}
    ppath = os.path.join(os.path.dirname(args.out), "sort_test_predictions.jsonl")
