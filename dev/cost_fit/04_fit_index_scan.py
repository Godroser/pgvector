#!/usr/bin/env python3
"""Index Scan fit: train 40 / test 10."""
...
def main() -> None:
    ...
    ap.add_argument("--data", default=os.path.join(os.path.dirname(__file__), "data", "index_scan_samples.jsonl"))
    ap.add_argument("--out", default=os.path.join(os.path.dirname(__file__), "models", "index_scan_coef.json"))
    ...
    payload = {"operator": "Index Scan", "features": names, "total_samples": len(rows), **ev}
    pred_path = os.path.join(os.path.dirname(args.out), "index_scan_test_predictions.jsonl")
