#!/usr/bin/env python3
"""Merge Join under SET LOCAL (hash + nestloop off)."""
...
def main() -> None:
    ...
    ap.add_argument("--out", default=os.path.join(os.path.dirname(__file__), "data", "mergejoin_samples.jsonl"))
    ...
    if n_ok < 50:
        print(f"[warn] only {n_ok} merge-join samples (<50).", file=sys.stderr)
