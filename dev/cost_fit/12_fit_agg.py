#!/usr/bin/env python3
"""Aggregate fit: train 40 / test 10."""
...
def main() -> None:
    ...
    ap.add_argument("--data", default=os.path.join(os.path.dirname(__file__), "data", "agg_samples.jsonl"))
    ap.add_argument("--out", default=os.path.join(os.path.dirname(__file__), "models", "agg_coef.json"))
    ...
    names = ["child_actual_rows", "actual_rows", "num_group_keys", "plan_width"]

    def xfn(r):
        return [r["child_actual_rows"], r["actual_rows"], r["num_group_keys"], r["plan_width"]]

    ev = fit_train_test_eval(rows, names, xfn, train_n=args.train_n, test_n=args.test_n, seed=args.seed)
    payload = {"operator": "Aggregate", "features": names, "total_samples": len(rows), **ev}
    ppath = os.path.join(os.path.dirname(args.out), "agg_test_predictions.jsonl")
